...
contract Create{
    ...
    Counters.Counter public _canditateId;
    ...
    function setCanditate(address _address, string memory _age, string memory _name, string memory _image, string memory _ipfs) public {
        require(votingOrganizer == msg.sender, "Only organizer can create the canditates");

        _canditateId.increment();

        uint256 idNumber = _canditateId.current();
        ...
        emit CanditateCreate(idNumber, _age, _name, _image, candidate.voteCount, _address, _ipfs);
    ...
    function getVoterLengh() public view returns (uint256){
        return votersAddress.length;
    }

    function getVotersdata (address _address) public view returns (uint256, string memory, string memory, address, string memory, uint256, bool){
        return(
